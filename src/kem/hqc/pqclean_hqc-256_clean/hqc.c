...
void PQCLEAN_HQC256_CLEAN_hqc_pke_encrypt(uint64_t *u, uint64_t *v, uint8_t *m, uint8_t *theta, const uint8_t *pk) {
    ...
    PQCLEAN_HQC256_CLEAN_vect_sample_fixed_weight_ctus(&vec_seedexpander, r2, PARAM_OMEGA_R);
    PQCLEAN_HQC256_CLEAN_vect_sample_fixed_weight_ctus(&vec_seedexpander, e, PARAM_OMEGA_E);
#elif HQC_SAMPLING_METHOD == 4
    // Method 4: Fixed-N
    PQCLEAN_HQC256_CLEAN_vect_sample_fixed_weight_fixed_n(&vec_seedexpander, r1, PARAM_OMEGA_R);
    PQCLEAN_HQC256_CLEAN_vect_sample_fixed_weight_fixed_n(&vec_seedexpander, r2, PARAM_OMEGA_R);
    PQCLEAN_HQC256_CLEAN_vect_sample_fixed_weight_fixed_n(&vec_seedexpander, e, PARAM_OMEGA_E);
#else
